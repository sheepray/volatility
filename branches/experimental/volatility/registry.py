...
class VolatilityCommandRegistry(MemoryRegistry):
    ...
    def install_method(self, cls_name, method_name, callable, replace = False):
        """ Installs the callable as a class method.

        This checks that the method does not already exist and if
        replace is not specified, raises it. Its basically a
        controlled way of monkey patching classes.
        """
        try:
            cls = self.commands[cls_name]
            if not replace and hasattr(cls, method_name):
                raise RuntimeError("%s.%s already exists..." % (cls_name, method_name))

            setattr(cls, method_name, callable)
        except KeyError:
            pass
...
def Init():
    ...
    global EVENTS
    EVENTS = VolatilityObjectRegistry(commands.EventHandler)

    if config.INFO:
        print_info()
        sys.exit(0)

    ## Now post the startup event to all handlers:
    for event_cls in EVENTS.classes:
        event = event_cls()
        event.startup()
